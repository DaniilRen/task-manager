...
def get_all_users(current_user_id):
	db = get_db()
	return db.execute(
			"SELECT id, first_name, second_name, surname, username, password, type"
			" FROM users WHERE id != ? AND type != 'admin';", (current_user_id,)
		).fetchall()
...
def add_new_user(fstn, secn, surn, login, psw, type):
	db = get_db()
	error = None
	missing = {fstn: 'Имя',
						secn: 'Фамилия',
	...
	if error is None:
			try:
					...
					db.execute(
							"INSERT INTO users"
							"(first_name, second_name, surname, username, password, type)"
							"VALUES (?, ?, ?, ?, ?, ?);",
							(fstn, secn, surn, login, hashed_psw, type)
						)
					db.commit()
